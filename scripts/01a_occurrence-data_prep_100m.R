...
#         Title: 01a occurrence data preparation - 100m
#         Project: OpenPrioBio
...
#         and spatial duplicates inside same 100m grid cell), sorting of 
#         pseudo-absences, thinning 200m (twice the grid cell size), then writes
...
# output directory
output_dir <- file.path("results/01a_occurrence-prep/100m")
...
obsp <- read.csv2(file.path(biodatafolder,
                            "GBIF_NRW_filtered_group1_3035.csv"),sep=",")
...
envdata <- 
  terra::rast(file.path(envdatafolder,"coregistration_2016_2024_100m/dgm_NRW.tif"))
...
# filter by accuracy -----------------------------------------------------------
      ...
      sub1_accuracy <- 
        subset_yr_spp[subset_yr_spp$coordinateUncertaintyInMeters <= 100, ]
...
# spatial thinning -------------------------------------------------------------
      ...
      thinned <- thin(Nodup_sf, thin_dist = 200, runs = 5, ncores = 5)
