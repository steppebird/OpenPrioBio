...
#         Title: 01a occurrence data preparation - 200m
#         Project: OpenPrioBio
...
#         and spatial duplicates inside same 200m grid cell), sorting of 
#         pseudo-absences, thinning 400m (twice the grid cell size), then writes
...
# output directory
output_dir <- file.path("results/01a_occurrence-prep/200m")
...
obsp <- read.csv2(file.path(biodatafolder,
                            "GBIF_NRW_filtered_group2_3035.csv"),sep=",")
...
envdata <- 
  terra::rast(file.path(envdatafolder,"coregistration_2016_2024_200m/dgm_NRW.tif"))
...
# filter by accuracy -----------------------------------------------------------
      ...
      sub1_accuracy <- 
        subset_yr_spp[subset_yr_spp$coordinateUncertaintyInMeters <= 200, ]
...
# absences are created so that they don't fall within cells with presence locations
...
# spatial thinning -------------------------------------------------------------
      ...
      thinned <- thin(Nodup_sf, thin_dist = 400, runs = 5, ncores = 5)
